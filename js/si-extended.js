...
// Style portfolio cards to match GitHub
document.querySelectorAll('.si-portfolio-card').forEach(function(card){
  var h=card.querySelector('h3,h4');if(!h)return;var n=h.textContent.trim();
  // Make company name uppercase
  h.style.textTransform='uppercase';h.style.letterSpacing='0.06em';h.style.fontSize='16px';h.style.fontWeight='700';h.style.color='#0A2342';
  // Fix badge colors: blue for current, grey for exit
  card.querySelectorAll('[class*="badge"]').forEach(function(b){
    var t=b.textContent.trim();
    if(t==='Huidig'||t==='Current'){b.style.cssText='display:inline-block;padding:4px 10px;border-radius:4px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;background:#e8f0fd;color:#1C5FA8;margin-top:12px'}
    if(t==='Exit'){b.style.cssText='display:inline-block;padding:4px 10px;border-radius:4px;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;background:#f0f0f0;color:#666;margin-top:12px'}
  });
  // Add tooltip
  if(tips[n]){card.style.position='relative';card.style.overflow='hidden';var tip=document.createElement('div');tip.className='si-card-tooltip';tip.textContent=tips[n];card.appendChild(tip)}
  // Add hover lift
  card.style.transition='box-shadow .25s ease,transform .25s ease,border-color .25s ease';
  card.onmouseenter=function(){card.style.boxShadow='0 6px 24px rgba(10,35,66,.14)';card.style.transform='translateY(-3px)';card.style.borderColor='transparent'};
  card.onmouseleave=function(){card.style.boxShadow='';card.style.transform='';card.style.borderColor=''};
});
// Hide duplicate filter card (first card that contains filter buttons)
var cards=document.querySelectorAll('.si-portfolio-card');
if(cards.length>0){
  var first=cards[0];var fh=first.querySelector('h3,h4');
  if(!fh||fh.textContent.trim()===''){first.style.display='none'}
  // Check if first card has filter-like text
  var ft=first.textContent.trim();
  if(ft.indexOf('Alle')>-1&&ft.indexOf('Huidig')>-1&&ft.indexOf('Exit')>-1){first.style.display='none'}
}
...
var ov=document.createElement('div');ov.className='si-fund-hover';var nm=document.createElement('div');nm.className='si-fund-hover-name';nm.textContent=n;ov.appendChild(nm);var ds=document.createElement('div');ds.className='si-fund-hover-desc';ds.textContent=fund.d;ov.appendChild(ds);if(fund.s&&fund.s.length){var ul=document.createElement('ul');ul.className='si-fund-hover-subs';fund.s.forEach(function(sub){var li=document.createElement('li');li.textContent=sub;ul.appendChild(li)});ov.appendChild(ul)}
// Add "Bezoek website" text at bottom
var lnk=document.createElement('div');lnk.textContent='Bezoek website \u2192';lnk.style.cssText='margin-top:auto;font-size:12px;font-weight:700;color:rgba(255,255,255,.9)';ov.appendChild(lnk);
card.appendChild(ov)});
}
